...
export async function fetchLinearIssueByIdOrIdentifier(idOrIdentifier: string) {
    const {token} = getConfig();
    if (!token) throw new Error("❌ Linear API 토큰이 설정되지 않았습니다.");
    ...
    if (idOrIdentifier.includes("-")) {
        // Likely an identifier (e.g., "ABC-123")
        query = `
      query {
    ...
    if (json.errors) {
        // If ID lookup failed, try searching by identifier
        const searchQuery = `
      query {
        issues(filter: { identifier: { eq: "${idOrIdentifier}" } }, first: 1) {
          nodes {
            id
            identifier
            title
    ...
    `;
        ...
        if (searchJson.errors || !searchJson.data.issues.nodes.length) {
            throw new Error(`❌ 해당 ID 또는 Identifier의 이슈를 찾을 수 없습니다: ${idOrIdentifier}`);
        }

        return searchJson.data.issues.nodes[0] as {
            id: string;
            identifier: string;
            title: string;
            description: string;
