...
program.name("vibe").description(t().program.description).version("3.0.0");
// ============ 통합 AI Agent 초기화 ============
program
    .command("init")
...
program
    ...
    try {
        ...
        const locale = t();
        console.error(chalk.red(locale.linear.errors.graphqlError.replace("{error}", error.message)));
    }
}))
    .addCommand(new Command("get").description(t().lang.get.description).action(() => {
    try {
